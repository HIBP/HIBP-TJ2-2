...
# %%
def discrepancy(coeffs, Iinj, traj, lam, ne_rec, Itot_interp, Te_interp,
                sigmaEff12_interp, sigmaEff13_interp,
                sigmaEff23_interp, Ibeam):
    ''' function reprents the difference between calculated current
    and experimental values
    '''
    I_calculated = Ibeam(coeffs, Iinj, traj, lam, ne_rec, Te_interp,
                         sigmaEff12_interp, sigmaEff13_interp,
                         sigmaEff23_interp)
    I_exp = Itot_interp(I_calculated[:, 2])
    discr = np.sum((I_exp/max(I_exp) - I_calculated[:, 3] /
                    max(I_calculated[:, 3]))**2)*10000  # *1e17

    # discr = np.mean((I_exp - I_calculated[:,3])**2)*1e17
    # mask = abs(I_calculated[:,3])<0.99
    # I_calculated = I_calculated[mask]
    # I_exp = Itot_interp(I_calculated[:,3])
...
# %%
def discrepancyI0(Iinj, coeffs, traj, lam, ne_rec, Itot_interp, Te_interp,
                  sigmaEff12_interp, sigmaEff13_interp,
                  sigmaEff23_interp, Ibeam):
    ''' function reprents the difference between calculated current
    and experimental values
    '''
    I_calculated = Ibeam(coeffs, Iinj*1e-6, traj, lam, ne_rec, Te_interp,
                         sigmaEff12_interp, sigmaEff13_interp,
                         sigmaEff23_interp)
    I_exp = Itot_interp(I_calculated[:, 2])
...
# %%
def ne_NoAtten(Iinj, Itot_interp, tr, lam, Te, NeShapeFunc, NeFit,
               neAvg, sigmaEff12):
    ...
    ne = np.zeros([0,4])  # [0]No [1]ne [2]A2 [3]rho
    # choose number of filaments
    fil_start = 3
    fil_stop = 3
    ...
    for i_No in Ntr:
        ...
        for i_fil in np.arange(fil_start, fil_stop+1, 1):
            ...
            if tr1.shape[0] > 0:
                print('\n *********Ntr={}, i_fil={}'.format(i_No, i_fil))
                n_fils += 1
                r_loc += tr1[0].rho_ion
                Ua2 = tr1[0].Ua2

        r_loc = r_loc/n_fils
        if (n_fils > 0) & (abs(r_loc) < 0.8):
            Te_loc = Te(r_loc)
            sigmaEff_loc = sigmaEff12(Te_loc)
#            print('n_fils={}, Te={:.2f}, r={:.2f}, sigma={}'.format(n_fils,
#                                          float(Te_loc), r_loc, sigmaEff_loc))
            ne_SV = Itot_interp(r_loc)/(2*Iinj*sigmaEff_loc*lam(r_loc))
#            ne[i_No - N_start, :] = [i_No, ne_SV*1e-19, Ua2, r_loc]
            ne = np.vstack([ne, [i_No, ne_SV*1e-19, Ua2, r_loc]])
...
# traj0 should be imported and then modified in import_ro_config.py
tr = np.recarray.copy(traj0)
...
# filename = 'D:\\NRCKI\\TJ-II_programs\\Itot\\' + str(shot) + '_ne04.dat'
...
kAmpl = 4*1e7  # amplification coefficient

# flag to optimize I0
optimizeI0 = False  # True

# flag to optimize ne
optimizeNe = False  # True
...
shotTS = 45784  #49867  #48441  # 48431  # 48428 #48441 #47152 #48441 #47152
t_TS = 1270  #1225 #1250  # 1150  # 1250
...
if optimizeI0:
    ...
    t1 = time.time()
    Iinj = Iinj*1e6
    result_I0 = optimize.minimize(discrepancyI0, Iinj,
                                  args=(coeffsNe, tr, lam_interp, ne_rec,
                                        Itot_interp, Te_interp,
                                        sigmaEff12_e_interp,
                                        sigmaEff13_e_interp,
                                        sigmaEff23_e_interp, Ibeam),
                                  method='BFGS',
...
if optimizeNe:
    ...
    result = optimize.minimize(discrepancy, coeffsNe,
                               args=(Iinj, tr, lam_interp, ne_rec,
                                     Itot_interp, Te_interp,
...
# %% plot ne and Itot
I_calculated = Ibeam(coeffsNe, Iinj, tr, lam_interp, ne_rec, Te_interp,
                     sigmaEff12_e_interp, sigmaEff13_e_interp,
                     sigmaEff23_e_interp)
...
plt.errorbar(I_calculated[:, 2], Itot_interp(I_calculated[:, 2]),
             xerr=np.full_like(I_calculated[:, 2], 0.1),
             yerr=np.full_like(Itot_interp(I_calculated[:, 2]), 0.5e-8),
             capsize=3, fmt='o', color='k', label='experiment')

I_calculated = Ibeam(coeffs_res, Iinj, tr, lam_interp, ne_rec,
                     Te_interp, sigmaEff12_e_interp, sigmaEff13_e_interp,
                     sigmaEff23_e_interp)
...
plt.plot(I_calculated[:, 2], I_calculated[:, 3]/max(I_calculated[:, 3]), '--o',
         color='b', label='initial iteration')
plt.errorbar(I_calculated[:, 2], Itot_interp(I_calculated[:, 2])/max(Itot_interp(I_calculated[:, 2])),
             xerr=np.full_like(I_calculated[:, 2], 0.1),
             yerr=np.full_like(Itot_interp(I_calculated[:, 2]), 0.1),
             capsize=3, fmt='o', color='k', label='experiment')

I_calculated = Ibeam(coeffs_res, Iinj, tr, lam_interp, ne_rec, Te_interp,
                     sigmaEff12_e_interp, sigmaEff13_e_interp,
                     sigmaEff23_e_interp)
...
kTS = ne_rec(0.0, *coeffs_res)/np.mean(Ne[maskTS][:, 1])
...
axs[1, 2].plot(I_calculated[:, 2], I_calculated[:, 3], '--o',
               color='b', label='initial iteration')
axs[1, 2].errorbar(I_calculated[:, 2], Itot_interp(I_calculated[:, 2]),
                   xerr=np.full_like(I_calculated[:, 2], 0.1),
                   yerr=np.full_like(Itot_interp(I_calculated[:, 2]), 0.5e-8),
                   capsize=3, fmt='o', color='k', label='experiment')
